...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnRunImport) {
        btnRunImport.addEventListener('click', async () => {
            // 1. Validação básica
            const externalUrl = apiUrlInput.value.trim(); // URL do sistema legado
            const externalKey = apiKeyInput.value.trim(); // Chave do sistema legado

            if (!externalUrl || !externalKey) {
                return showToast('Configure a URL e a Chave da API antes de importar.', 'error');
            }

            // UI de carregamento
            const originalText = btnRunImport.innerHTML;
            btnRunImport.disabled = true;
            btnRunImport.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Processando...';

            try {
                // PASSO A: Buscar dados do Sistema Legado (server.js)
                // Nota: O server.js espera a chave no header 'x-api-key'
                const responseLegado = await fetch(externalUrl, {
                    method: 'GET',
                    headers: { 
                ...
                if (!responseLegado.ok) {
                    throw new Error('Falha ao conectar com o Sistema Legado (Verifique URL/Chave).');
                }

                const dadosLegado = await responseLegado.json();
                
                // O server.js retorna: { "usuarios": [...] }
                // O Laravel espera: { "usuarios": [...] }
                // As estruturas são compatíveis.

                // PASSO B: Enviar para o Laravel (UsuarioController.php)
                const responseLaravel = await fetch(`${API_BASE_URL}/api/usuarios/import`, {
                    method: 'POST',
                    headers: { 
                        'Authorization': `Bearer ${authToken}`, 
                        'Content-Type': 'application/json',
                    ...
                    body: JSON.stringify(dadosLegado) // Envia o objeto inteiro contendo a lista
                });

                if (responseLaravel.ok) {
                    const resultado = await responseLaravel.json();
                    showToast(resultado.message || 'Importação realizada com sucesso!');
                } else {
                    const erroLaravel = await responseLaravel.json();
                    throw new Error(erroLaravel.message || 'Erro ao processar importação no servidor.');
                }
